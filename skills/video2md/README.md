...
| 要能被程序读的结构化转写（做剪辑 / 对齐 / 再生成字幕） | 「输出 JSON」 | `--emit-json` |

**词级时间戳有什么用**：每个词都带置信度，能一眼看出哪个词机器没听准。比如置信度 0.038 的词，八成是错的，值得回去听一遍。

**`--emit-json` 有什么用**：额外产出一份 `会议录屏.transcript.json`（段 / 词级时间戳、置信度、说话人），给程序直接读——做剪辑、做对齐、再生成字幕，都不用去解析 Markdown。默认不产出，跟 AI 说一句「输出 JSON」即可。
